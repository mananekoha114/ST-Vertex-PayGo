...
远端文件是本项目根据 Google 官方 [Flex PayGo](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/flex-paygo) 与 [Priority PayGo](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/priority-paygo) 文档维护的机器可读镜像，并不是 Google 官方提供的 GitHub 数据源。载入前会完整校验 schema、日期、模型 ID、文件大小以及历史模型集合，只有整份数据有效时才会一次性替换内存策略并写入缓存。

维护名单时请同步更新 `updatedAt`、`tiers` 和 `knownModels`。`knownModels` 是只增不减的历史集合：模型即使从两个层级都下架，也必须继续保留在这里作为 tombstone，避免被误判为尚未收录的新模型。`main` 分支中的这份文件是运行时策略控制面，合并后会直接影响客户端判定，建议为分支启用保护规则并要求人工审核名单变更。

---

## 安全与防坑机制

- 🔒 **密钥安全**：扩展仅在浏览器端工作，不会偷存或上传你的 Google Key/服务账号；GitHub 名单请求明确不携带凭据，Vertex 认证仍完全由酒馆本地管理。
